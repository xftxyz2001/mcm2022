...
# print(current_day)
fpout = open('A24avg.csv', 'w')  # gbk
fpout.write('日期,有效时间数,平均温度(℃),平均湿度(%),平均气压(MBar),平均风速(m/s),平均风向(°)\n')
fpwarnings = open('监测点A日平均warnings.txt', 'w', encoding='utf-8')
...
csv = pd.read_csv('A24avg.csv', encoding='gbk')
# 把 csv 文件转换为 Excel 文件，并把数据保存在创建的表格 data 中
csv.to_excel('监测点A日平均.xlsx', sheet_name='data')
